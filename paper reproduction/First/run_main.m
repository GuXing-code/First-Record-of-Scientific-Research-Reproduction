...
% 信道与噪声
h = [zeros(1,50),1, zeros(1,99), 0.8, zeros(1,80), 0.6];

rx_signal = conv(rx_signal, h);
rx_signal = awgn(rx_signal, 5, 'measured');
...
%信道估计（基于复符号序列）
h_pre = channel_estimation(z_train, z_rx, 20, 500);

%理论的h（解调后）：此函数完全由ai生成
[h_eq_true, tap_axis_true, detail_true] = true_equivalent_channel( ...
    h, fc, Rs, Ts, length(h_pre), lfm_ref, [], 'natural');
